...
/**
 * todo 做成工具类,满足需求,但不是完备的工具类,后续再优化
 *
 * @ Desc 使用SMTP协议发送电子邮件
 * @ Author YoungSmith
 * @ Date 17-10-25 下午4:41
 */
...
//
//    // 发件人
//    private static String from = "young_smith@163.com";
//
//    // 收件人
//    private static String to = "shiyang@internetware.cn";
//
//    // 初始化连接邮件服务器的会话信息
...
//     * 发送简单的文本邮件
//     */
//    public static boolean sendTextEmail(Email email) throws Exception {
//        try {
//            // 创建Session实例对象
//            Session session1 = Session.getDefaultInstance(email.getProps());
//
//            // 创建MimeMessage实例对象
...
//            // 设置收件人
//
//            message.setRecipient(RecipientType.TO, new InternetAddress(email.getReceivers().get(0).getEmail()));
//            // 设置发送时间
...
//    }
     ...
     * 发送简单的html邮件
     */
    public static boolean sendHtmlEmail(Email email) throws Exception {
        // 创建Session实例对象
        Session session1 = Session.getInstance(email.getProps(), email.getSender());
        ...
        message.setSentDate(new Date());
        // 设置收件人
        message.setRecipients(RecipientType.TO, InternetAddress.parse(email.getReceivers().get(0).getEmail()));
        ...
        String nick;
        try {
            nick = javax.mail.internet.MimeUtility.encodeText(email.getSender().getNick());
        } catch (UnsupportedEncodingException e) {
            throw new EmailExeception(e);
        }
        message.setFrom(new InternetAddress(nick + " <" + email.getSender().getEmail() + ">"));
        // 保存并生成最终的邮件内容
        message.saveChanges();
...
//
//        private String username = "young_smith@163.com";
//
//        private String password = "shiyang0511";
//
